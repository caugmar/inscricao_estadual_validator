...
class InscricaoEstadualValidatorTest < Test::Unit::TestCase
  def test_black_list
  ...
  def assert_valid(inscricao_estadual)
    assert company(:inscricao_estadual => inscricao_estadual).valid?
  end

  def assert_invalid(inscricao_estadual)
    assert company(:inscricao_estadual => inscricao_estadual).invalid?
  end
